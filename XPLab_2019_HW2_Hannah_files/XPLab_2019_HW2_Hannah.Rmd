...
author: "Sarah Greb( 973152),Hannah Terborg (969021),Alexander Palatnik 973099"
output:
  html_document
---
...
ToothGrowth%>%subset(supp == "OJ")%>%as_tibble()%>%print()
...
ToothGrowth%>%subset(dose >= 1.0 & supp == "VC")%>%as_tibble()%>%print()
...
ToothGrowth%>%as_tibble()%>%group_by(supp)%>%summarise(mean=mean(len))%>%print()
...
ToothGrowth%>%ggplot(aes(dose, len))+ geom_point(aes(color=supp))
...
Orange juice seems to be more helpfull comparing small doses of both, when giving a dose of 2.0 tho there seems to be no differnece. 
